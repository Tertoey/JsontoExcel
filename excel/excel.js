...
async function writeToExcel(data) {
    ...
    try {
        ...
        if (lastRowNumber === 0) {
            ...
            sheet.cell(1, 2).value('Noise Level');
            sheet.cell(1, 3).value('PM2.5');
            ...
            sheet.cell(lastRowNumber + 2, 2).value(data.rssi);
            sheet.cell(lastRowNumber + 2, 3).value(data.temp);
            sheet.row(lastRowNumber + 2).style({ horizontalAlignment: 'left' });
        }else{
            // Add the new data to the worksheet
            sheet.cell(lastRowNumber + 1, 1).value(data.datetime);
            sheet.cell(lastRowNumber + 1, 2).value(data.rssi);
            sheet.cell(lastRowNumber + 1, 3).value(data.temp);
